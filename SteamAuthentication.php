...
class SteamAuthentication {
	public $settings = array(
		'description' => 'Allow your users to login to your website using Steam. They will be presented with a page to enter their name and email address for the first time when logging in which will create an account and bind it to their steam account.',
	);
	public $loggedinURL = '/';
	function loginbutton($buttonstyle = "square") {
		return '<a href="/User/SteamAuthentication/"><img src="http' . (isset($_SERVER['HTTPS']) ? 's' : '') . '://steamcommunity-a.akamaihd.net/public/images/signinthroughsteam/sits_large_border.png" alt="Sign in through Steam"></a>';
	}
	function handleLogin($user_row) {
		global $billic, $db;
		...
		// Todo: load any module with an after_login function
		...
		$billic->redirect($this->loggedinURL);
	}
	function user_area() {
		global $billic, $db;
		...
		if (empty($_SESSION['steamid'])) {
			$steamauth['apikey'] = get_config('SteamAuthentication_API_Key');
			$steamauth['domainname'] = preg_replace('~^www\.~', '', $_SERVER['SERVER_NAME']);
			try {
				...
				if ($openid->mode) {
					...
					$steamid = $matches[1];
					$user = $db->q('SELECT * FROM `users` WHERE `steamid` = ?', $steamid);
					$user = $user[0];
					if (!empty($user)) {
					...
					$billic->redirect($openid->authUrl());
				}
			}
		...
		if (isset($_POST['register'])) {
			if (empty($_POST['username'])) {
				$billic->error('Username can not be empty', 'username');
			} else if (!ctype_alpha($_POST['username'])) {
				$billic->error('Username can only be alphabetic characters', 'username');
			}
			...
			if (empty($billic->errors)) {
				...
				$user = $db->q('SELECT * FROM `users` WHERE `id` = ?', $billic->userid);
				$user = $user[0];
				$this->handleLogin($user);
			}
		}
		...
		echo '<tr><th colspan="2">Complete your registration</th></tr>';
		echo '<tr><td' . $billic->highlight('username') . '>Username:</td><td><input type="text" class="form-control" name="username" value="' . safe($_POST['username']) . '"></td></tr>';
	...
	function settings($array) {
		...
		if (empty($_POST['update'])) {
			echo '<form method="POST"><input type="hidden" name="billic_ajax_module" value="SteamAuthentication"><table class="table table-striped">';
			echo '<tr><th>Setting</th><th>Value</th></tr>';
			echo '<tr><td>Steam API Key</td><td><input type="text" class="form-control" name="SteamAuthentication_API_Key" value="' . safe(get_config('SteamAuthentication_API_Key')) . '"> Your Steam WebAPI-Key found at http://steamcommunity.com/dev/apikey</td></tr>';
			echo '<tr><td colspan="2" align="center"><input type="submit" class="btn btn-default" name="update" value="Update &raquo;"></td></tr>';
			echo '</table></form>';
		} else {
			if (empty($billic->errors)) {
				set_config('SteamAuthentication_API_Key', $_POST['SteamAuthentication_API_Key']);
...
if (!class_exists('LightOpenID')) {
	...
	class LightOpenID {
		...
		function validate() {
			...
			$test = explode('?', $this->returnUrl);
			$test = $test[0];
			if ($this->data['openid_return_to'] != $test) {
